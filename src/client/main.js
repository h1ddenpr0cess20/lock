...
import { createLock } from './padlock/index.js';
...
const lock = createLock({ stage, THREE });
...
function chipState() {
  if (!session.connected || !session.muted) return lock.state;
  return lock.state === 'listening' || lock.state === 'idle' ? 'muted' : lock.state;
}
...
session.on('level', (level) => lock.setLevel(level));
session.on('pulse', (weight) => lock.pulse(weight));
...
/** Reaching for something pops the shackle: the one unlock that is on cue
 *  rather than on a whim. */
session.on('tool', (label) => {
  hud.setTool(label);
  if (label) lock.unlock();
});
...
session.on('interrupted', () => lock.anger(0.9));
