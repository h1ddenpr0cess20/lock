...
function taskNote(task) {
  ...
  switch (task.status) {
    case 'done':
      return `[lock] ${what} finished after ${task.ran_for}. It reports: ${task.summary}`;
    case 'cancelled':
      return `[lock] ${what} was stopped after ${task.ran_for}.`;
    case 'timeout':
      return `[lock] ${what} was still going after ${task.ran_for} and was stopped.`;
    default:
      return `[lock] ${what} failed after ${task.ran_for}. ${task.error ?? ''}`.trim();
  }
}
